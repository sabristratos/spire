...
<div class="flex flex-col gap-1 min-w-[4rem]">
    ...
    <button
        ...
        class="flex items-center justify-center h-6 text-muted hover:text-text active:scale-95 transition-all"
    >
            ...
            >
                <template x-for="(item, index) in {{ $type }}Options" :key="item.value">
                    <div
                        ...
                        :class="{ 'font-bold text-text': {{ $type }} === item.value, 'text-muted': {{ $type }} !== item.value }"
                        role="option"
