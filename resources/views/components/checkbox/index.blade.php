...
@if($variant === 'regular')
    <label
        for="{{ $checkboxId }}"
        {{ $attributes->merge(['class' => $containerBuilder->build()])->whereDoesntStartWith('wire:model')->whereDoesntStartWith('x-model')->except(['id', 'wire:key', 'indeterminate', 'checked', 'disabled', 'required', 'name', 'value', 'label', 'description', 'x-bind:checked', 'x-bind:disabled']) }}
